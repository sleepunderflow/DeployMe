...
void extractConfig(int argc, char **argv) {
  // If something fails we either already have the language set by user or we
  // have the default set so we can show the message
  for (int i = 1; i < argc; i++) {
    std::string argument = std::string(argv[i]);
    if (argument == "-l" || argument == "--language") {
      assertOrFailWithMessage(
          (i + 1 < argc),
...
void showUsage(char *name) {
  std::cout << "Usage: " << name << " [parameters]\n\n"
            << "Parameters:\n"
