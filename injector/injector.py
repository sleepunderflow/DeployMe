...
class fileToInject:
  ...
  def setPermissions(self, permissions):
    ...
    or permissions[2] not in allowedPermissionCharacters:
      raise ValueError("Incorrect file permission format. Expected format is the numeric format used by chmod (for example 755). Got {0}".format(permissions))
  ...
  def print(self):
    ...
    print("ID = {4}, Name = {0}, Path = {1}, Permissions = {2}, Flags = {3}".format(self.name, self.path, self.permissions,self.flags, self.ID))
  ...
  def parseConfig(self, key, value):
    ...
    if key == 'path':
      ...
      raise ValueError("Wrong configuration options for an [Item] {0}:{1}".format(key, value))
...
def parseConfigFile(line):
  ...
  if line == '':
    ...
    if currentlyBeingParsed == None:
      raise ValueError("Configuration file parsing error! Either not yet supported item being configured or [...] header not specified")
    pos = line.find('=')
    key = line[0:pos]
    value = line[pos+2:-1]
